...
# import results josn
acc_range_results = pd.read_json('<PATH_TO_JSON_FILE>')

# transform to csv 
acc_range_results.to_csv('<PATH_TO_OUTPUT_CSV_FILE>', header=True,  index=False, encoding='utf-8')

#print(acc_range_results.head())
print(len(acc_range_results)) 
...
# write to csv
final_df.to_csv(path + "acc_testing_shares.csv", header=True,  index=False, encoding='utf-8')
